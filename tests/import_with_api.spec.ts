...
test("Import via API", async () => {
  ...
  const fileEncodedBase64 = fs.readFileSync(
  ...
  expect(importData["Errors"][0]["TranslationEntity"]).toEqual(
    expect.objectContaining({ Key: "ImportInvalidEmploymentTemplate" })
  );
});
